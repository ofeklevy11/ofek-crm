...
export async function consumeNurtureQuota(
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    return { allowed: false, remaining: 0, resetInSeconds: WINDOW_SECONDS };
  }
  ...
  try {
    ...
    // Redis down — DENY request to prevent over-sending in multi-instance deployments
    return {
      allowed: false,
      remaining: 0,
...
export async function consumeNurtureQuotaBulk(
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    return { consumed: 0, remaining: 0, resetInSeconds: WINDOW_SECONDS };
  }
  if (!TIER_LIMITS.hasOwnProperty(tier)) {
    return { consumed: 0, remaining: 0, resetInSeconds: WINDOW_SECONDS };
  }
  ...
  try {
    ...
    // Redis down — DENY to prevent over-sending in multi-instance deployments
    return { consumed: 0, remaining: 0, resetInSeconds: WINDOW_SECONDS };
  }
}
...
export async function getNurtureQuotaStatus(
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    return { used: 0, limit: 0, remaining: 0, resetInSeconds: 0, tier };
  }
  if (!TIER_LIMITS.hasOwnProperty(tier)) {
    return { used: 0, limit: 0, remaining: 0, resetInSeconds: 0, tier };
  }
