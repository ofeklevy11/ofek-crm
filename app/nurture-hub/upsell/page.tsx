...
import { UPSELL_SMART_FIELDS } from "@/lib/nurture-fields";
...
export default function UpsellAutomationPage() {
  ...
  const handleBulkConfirmSend = async (channels: ChannelSelection, subscriberIds?: string[]) => {
    ...
    setSending(true);
    try {
      const result = await sendNurtureCampaign("upsell", undefined, channels, subscriberIds);
      if (result.success) {
  ...
  const handleConfirmSend = async (selectedChannels: ChannelSelection) => {
    ...
    try {
      const result = await sendNurtureCampaign("upsell", customer.id, selectedChannels);
      if (result.success) {
        const ch = result.channelsSent;
        const parts: string[] = [];
        if (ch?.sms) parts.push("SMS");
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="mr-auto flex items-center gap-3">
            <div className="flex items-center gap-2 border rounded-lg px-3 py-1.5">
              <Label htmlFor="upsell-enabled" className="text-sm text-slate-600 cursor-pointer">
                {isEnabled ? "פעיל" : "כבוי"}
              </Label>
              <Switch
                id="upsell-enabled"
            ...
            <Card>
              ...
              <CardContent>
                <NurtureSubscriberSearch
                  smartFields={UPSELL_SMART_FIELDS}
                  search={search}
