...
export default function WinbackAutomationPage() {
  ...
  const handleBulkConfirmSend = async (channels: ChannelSelection, subscriberIds?: string[]) => {
    ...
    setSending(true);
    try {
      const result = await sendNurtureCampaign("winback", undefined, channels, subscriberIds);
      if (result.success) {
  ...
  const handleConfirmSend = async (selectedChannels: ChannelSelection) => {
    ...
    try {
      const result = await sendNurtureCampaign("winback", customer.id, selectedChannels);
      if (result.success) {
        const ch = result.channelsSent;
        const parts: string[] = [];
        if (ch?.sms) parts.push("SMS");
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-2">
              <UserPlus className="w-8 h-8 text-slate-600" aria-hidden="true" />
