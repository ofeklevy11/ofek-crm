...
export function ProductModal({
  ...
  return (
    ...
    >
      <ProductModalContent onClose={onClose} productToEdit={productToEdit} />
    </div>
  );
}
...
function ProductModalContent({
  ...
  const containerRef = useFocusTrap(onClose, !descriptionPopupOpen);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label htmlFor="product-name" className="block text-sm font-medium text-gray-700">
              שם הפריט <span className="text-rose-500" aria-hidden="true">*</span>
            </label>
            <input
              id="product-name"
              required
        ...
        <div className="space-y-2">
          <label htmlFor="product-description-btn" className="block text-sm font-medium text-gray-700">
            תיאור
          </label>
          <button
            ...
            className="w-full px-4 py-2.5 border border-gray-200 rounded-xl hover:border-[#4f95ff] outline-none focus:ring-2 focus:ring-[#4f95ff] focus:border-[#4f95ff] transition-all bg-white text-right flex items-center justify-between gap-2"
          >
            <span className={`truncate ${formData.description ? "text-gray-900" : "text-gray-400"}`}>
              {formData.description || "לחץ להוספת תיאור..."}
            </span>
            <svg aria-hidden="true" className="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label htmlFor="product-price" className="block text-sm font-medium text-gray-700">
              מחיר (הכנסה) <span className="text-rose-500" aria-hidden="true">*</span>
            </label>
            <div className="relative">
              <span className="absolute right-4 top-2.5 text-gray-500" aria-hidden="true">
                ₪
              </span>
              <input
                id="product-price"
...
function DescriptionPopupContent({
  onClose,
  description,
  onDescriptionChange,
}: {
  ...
  return (
    <div ref={containerRef} className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh]">
      <div className="flex justify-between items-center p-5 border-b border-gray-100">
        <h3 id="description-popup-title" className="text-lg font-semibold text-gray-900">תיאור המוצר / שירות</h3>
        <button
          type="button"
          onClick={onClose}
